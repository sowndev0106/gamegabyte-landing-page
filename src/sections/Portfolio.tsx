...
import { featuredWork, workIndex, workPath } from '../content/work/types'
 ...
 *
 * Which three is a data question, answered by `featured` in
 * `src/content/work/overrides.json`. The full archive lives at `/work/`.
 */
export function Portfolio() {
  ...
  return (
    <Section id="portfolio">
      <Container>
        <SectionSplit layout="stacked" id="portfolio" title={content.portfolio.title} description={content.portfolio.intro} >
          ...
          {/* Three is the shop window; the count says how much is behind it. */}
          <Reveal>
            <a
              href="/work/"
              className="group mt-3.5 flex items-center justify-between border border-white/11 px-5.5 py-5 transition-colors hover:border-accent/40"
            >
              <span className="font-mono text-[9px] uppercase tracking-[0.22em] text-white/48 group-hover:text-white">
                View all {workIndex.length} files
              </span>
