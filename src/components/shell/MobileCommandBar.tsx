 ...
 */
export function MobileCommandBar({ active, base = '' }: { active: string; base?: string }) {
  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-50 flex h-17.5 items-center justify-between border-b border-white/11 bg-ink/95 px-4.5 backdrop-blur-xl md:hidden">
        <a href={`${base}#home`} className="font-display text-[15px] font-extrabold uppercase tracking-[0.08em]">
          {content.shell.brand}
        </a>
        <button
          type="button"
